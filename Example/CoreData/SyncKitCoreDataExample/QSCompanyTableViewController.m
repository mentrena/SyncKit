...
- (IBAction)synchronize:(id)sender
{
    ...
    [self.synchronizer synchronizeWithCompletion:^(NSError *error) {
        ...
        if (error) {
            ...
            CKRecordZoneID *zoneID = weakSelf.synchronizer.changeManagers.firstObject.recordZoneID;
            [weakSelf.synchronizer subscribeForChangesInRecordZone:zoneID withCompletion:^(NSError *error) {
                if (error) {
        ...
        sharingController = [[UICloudSharingController alloc] initWithPreparationHandler:^(UICloudSharingController * _Nonnull controller, void (^ _Nonnull preparationCompletionHandler)(CKShare * _Nullable, CKContainer * _Nullable, NSError * _Nullable)) {
            ...
            [self.synchronizer synchronizeWithCompletion:^(NSError *error) {
                if (error) {
                    ...
                    [self.synchronizer shareObjectWithIdentifier:company.identifier publicPermission:CKShareParticipantPermissionReadWrite participants:@[] completion:^(CKShare *share, NSError *error) {
...
{
    [self.synchronizer saveShare:csc.share forObjectWithIdentifier:self.sharingCompany.identifier];
    [self.tableView reloadData];
}

- (void)cloudSharingControllerDidStopSharing:(UICloudSharingController *)csc
{
    [self.synchronizer deleteShareForObjectWithIdentifier:self.sharingCompany.identifier];
    [self.tableView reloadData];
}
